...
def update(id, song, artist, album, year):
    conn = sqlite3.connect("songs.db")
    cur = conn.cursor()
    cur.execute("UPDATE song SET song=?, artist=?, album=?, year=? WHERE id=?", (song, artist, album, int(year), int(id))) # don't forget the comma or python will not interptret this as a tuple
    conn.commit()
    conn.close()
